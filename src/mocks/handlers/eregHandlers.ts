...
export const eregHandlers = (demoprofil: Demoprofil) => [
    http.post<{}, { orgnr: string }>('/min-side-arbeidsgiver/api/ereg/underenhet', async ({ request }) => {
    ...
    http.post<{}, { orgnr: string }>('/min-side-arbeidsgiver/api/ereg/overenhet', async ({ request }) =>
        HttpResponse.json({
