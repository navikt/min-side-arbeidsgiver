...
import { HearingLoopIcon } from '@navikt/aksel-icons';
...
const AccessibleIcon = ({
    ...
    ...props
}: { title: string; 'aria-hidden': boolean } & React.SVGProps<SVGSVGElement>) => {
    const id = React.useId();
    return (
        <svg {...props} aria-hidden={ariaHidden} focusable="false" role="img" aria-labelledby={id}>
...
export const StipletTidslinjeLinjeIkon = ({ height = 32 }: { height?: number }) =>
    TidslinjeLinjeIkon({ title: 'Stiplet linje', height, stiplet: true });

export const DelvisStipletTidslinjeLinjeIkon = ({ height = 32 }: { height?: number }) =>
    TidslinjeLinjeIkon({ title: 'Delvis stiplet linje', height, delvis: true });

export const SolidTidslinjeLinjeIkon = ({ height = 32 }: { height?: number }) =>
    TidslinjeLinjeIkon({ title: 'Linje', height });
